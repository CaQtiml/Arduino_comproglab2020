...
extern unsigned char logo[]={'P','O','N','G'};
int screen_width = 320;
int screen_height = 240;
int raket_position1,raket_position2;
long interval;
int ball_x;
int ball_y;
int ball_r;
...
void game() {
  ...
  while (true)
  {
    ...
    raket_position1 = map(percentage1, 0, 100, 0, 5);
    raket_position2 = map(percentage2, 0, 100, 5, 10);
    int raket_width = 20;
    int raket_height = 40;
    int raket_margin = 10;
    ...
    for (int i = 0; i < 10; i++)
    {
      ...
      if (i == raket_position1)
      {
        raket1_x = x;
        raket1_y = y;
      }
      else if(i== raket_position2)
      {
        raket2_x = x;
        raket2_y = y;
      }
    ...
    if (currentMillis - previousMillis > interval) {
      ...
      if (ball_y >= 240)
        accel_y *= -1;    
      //if (raket_position1 > 4 || raket_position2 > 4)
      //{
        if ((ball_x + ball_r >= raket1_x) && (ball_y + ball_r >= raket1_y) && (ball_y - ball_r <= raket1_y + raket_height) && raket_position1 > 4)
        {
          accel_x *= -1;
          accel_y *= 1;
          repulse();
        }
        else if ((ball_x + ball_r >= raket2_x) && (ball_y + ball_r >= raket2_y) && (ball_y - ball_r <= raket2_y + raket_height) && raket_position2 > 4)
        {
          accel_x *= -1;
          accel_y *= 1;
          repulse();
      ...
      //else
      //{
        if ((ball_x - ball_r <= raket1_x + raket_width) && (ball_y + ball_r >= raket1_y) && (ball_y - ball_r <= raket1_y + raket_height) && raket_position1 < 4) 
        {
          accel_x *= -1;
          accel_y *= 1;
          repulse();
        }
        else if ((ball_x - ball_r <= raket2_x + raket_width) && (ball_y + ball_r >= raket2_y) && (ball_y - ball_r <= raket2_y + raket_height) && raket_position2 < 4) 
        {
          accel_x *= -1;
          accel_y *= 1;
          repulse();
